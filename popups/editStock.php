<div class="bg1 editStock<?= $row['stId']?> hidden fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center px-4">
    ...
    <form action="api/ProductController.php?id=<?= $row['stId']?>" method="POST"
        ...
        <div class="flex justify-between items-center border-b pb-3">
            <div>
                <h1 class="text-lg font-semibold">Edit Stock Batch</h1>
                <p class="text-sm text-gray-500">for <?= $row['name']?></p>
            </div>

            <button type="button" onclick="closeEdit(<?= $row['stId']?>)"
