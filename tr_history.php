        ...
        <div class="w-full h-full bg-white shadow-md rounded-sm p-4 flex flex-col gap-4">
            <div class="w-full h-10rem flex justify-between">
                <div class="w-auto h-auto flex items-center font-medium">
                    <a href="transactions.php" class="px-2 hover:bg-slate-100 py-1 border-b-2">Transactions Today</a>
                    <a href="tr_history.php" class="px-2 hover:bg-slate-100 py-1 border-b-2 border-green-600 bg-slate-100">Transaction History</a>

                    <button class="trig flex items-center gap-2 ml-2 px-2 py-1 bg-neutral-200 rounded-sm hover:bg-neutral-300 active:opacity-80">Filter Date <img src="imgs/chev.png" alt="" class="w-3 h-3"></button>
            ...
            <div class="w-full h-[80%] overflow-y-auto">
                <table class="w-full font-medium" id="myTable">
                    <thead>
                        <tr class="border-y bg-green-600 text-white">
                            ...
                            <th class="p-1 text-left">Date and Time </th>
                        ...
                        <?php
                            ...
                            if(mysqli_num_rows($run_query)>0){
                                while($row = mysqli_fetch_array($run_query)){
                                    ...
                                    $trTime = date("m/d/Y h:i:s A", strtotime($trTime));
                ...
                <div class="w-full h-auto flex items-center justify-end py-2 px-2 gap-2">
                    <a href="tr_history.php?page=<?= isset($_GET['page']) ? $_GET['page'] - 1 : -1;?>"><button class="bg-slate-200 p-1 rounded-sm hover:bg-slate-300 font-medium active:opacity-80">< Previous</button></a>
                    <a href="tr_history.php?page=<?= isset($_GET['page']) ? $_GET['page'] + 1 : 1;?>"><button class="bg-slate-200 p-1 rounded-sm hover:bg-slate-300 font-medium active:opacity-80">Next ></button></a>
                </div>
                <?php }?>
            </div>
            <div class="w-full h-[10%] flex justify-between gap-4">
                ...
                <div class="w-1/5 h-full flex font-medium rounded-sm p-2 gap-4">
                    <?php 
                        ...
                        ?>
                    <h1 class="text-lg">Total sales this day: <br> <?= date("F j, Y", strtotime($date));?></h1>
